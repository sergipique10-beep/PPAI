---
name: mobile
description: Capacitor plugins, builds Android, permisos nativos y signing APK para proyectos PPAI.
---
...
## Ponytail — antes de agregar un plugin nativo

Antes de instalar cualquier plugin de Capacitor:

1. ¿La Web API del browser ya lo cubre (Camera API, Geolocation, Notifications)? → Usarla, sin plugin.
2. ¿Capacitor Core ya incluye esto (`@capacitor/core`)? → No instalar plugin separado.
3. ¿Un plugin ya instalado en el proyecto lo resuelve? → Reutilizarlo.
4. Solo entonces: instalar el plugin minimo necesario.

Declarar solo los permisos de Android que el flujo actual usa. No agregar permisos "por si acaso". Cada plugin nuevo es superficie de ataque y peso en el APK — justificarlo.
