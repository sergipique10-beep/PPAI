---
name: database
description: Esquemas PostgreSQL, migraciones, queries optimizadas y configuracion Supabase dev para proyectos PPAI.
---
...
## Ponytail — antes de tocar el esquema

Antes de crear una tabla, columna o index:

1. ¿Una tabla existente puede extenderse con una columna? → No crear tabla nueva.
2. ¿PostgreSQL ya lo resuelve built-in (arrays, JSONB, generated columns, vistas)? → Usarlo.
3. ¿El index ya existe o puede ser el PK? → No duplicar.
4. Solo entonces: el minimo schema que cubre el caso pedido.

No crear tablas "por si acaso se necesita despues". No indexar columnas que no aparecen en WHERE ni JOIN todavia. Si un query se puede escribir en una sola sentencia sin CTE intermedia, hacerlo asi. Toda migracion no trivial incluye su rollback o es idempotente.
